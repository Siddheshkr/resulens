...
| Motion                   | Motion 13.4.0 for isolated client-side route, reveal, and hero interactions                                 |
| Validation               | Zod 4.5                                                                                                     |
...
- The product shell supports Light, Dark, and System appearance across public, auth, and protected surfaces. System is the default and follows live device changes; an explicit browser-local preference persists across navigation/reloads and synchronizes between tabs. A pre-paint script restores the palette. Appearance lives alongside Settings and sign-out in the account menu; visitors have a header appearance control. Semantic colors, responsive transparent imagery, accessible focus, and reduced motion follow `design.md`.
- Clerk account components use the hosted Clerk UI bundle through `ClerkProvider`; ResuLens keeps its theme-aware appearance in namespaced `appearance.elements` classes and does not ship the vulnerable bundled `@clerk/ui` dependency.
...
- The Supabase Edge Function `process-resumes` consumes at most three durable processing jobs per invocation and calls the server-only Node processor. The processor re-checks deletion state immediately before writing profile data; deletion cascades profile and queue rows and removes the private object. Retry resets are conditional on the current worker lock, and worker responses contain aggregate statuses only and never return resume or embedding identifiers.
- For Docker-free local development, `scripts/run-local-workers.mjs` provides the same bounded queue delivery contract: it reads opaque queue messages with the service role, calls the localhost internal route, and acknowledges only terminal results. The Supabase Edge Function emulator still requires Docker or Podman and is not part of this local workflow.
...
- The authenticated local browser walkthrough exercised upload-without-file validation, scan-history navigation, profile review/edit entry and return, retention preference changes, and the disabled account-delete guard. Destructive resume/account deletion was intentionally not invoked.
...
- The hosted development project now has one active curated Adzuna source row (`india-software-engineering`, India, `software engineer`, six-hour refresh). The row contains only non-secret provider configuration; Adzuna application credentials remain server-only environment values and were not written to the repository or database. The local-first path is verified independently of Vercel: `.env.local` contains the service-role key, worker secret, and Adzuna credentials; localhost health returns 200; a bounded local ingestion run normalized 100 records with 0 failures and reached the five-page safety cap; the hosted database contains 101 active postings; and `/dashboard/jobs` renders normalized listings. Hosted `ingest-jobs` Edge Function version 4 is active with JWT gateway verification and hosted `pg_cron`/`pg_net` are enabled. On 2026-09-21, the Vercel Production Supabase publishable-key and service-role configuration was corrected and Dashboard, Jobs, Matches, and Settings rendered on the new deployment. The six-hour ingestion schedule remains paused pending a separate hosted worker smoke test.
...
- The local authenticated browser walkthrough rendered the active listing feed, searched `Angular`, applied country/workplace filters, showed the intentional empty state, opened a normalized job detail, and preserved canonical source-link behavior.
...
- Approved resume profiles are converted into bounded professional-content embeddings that intentionally exclude names, email, phone numbers, locations, and raw evidence excerpts. Normalized job content is embedded separately. Supabase Queues carries only opaque embedding-job identifiers; the Edge Function and Node processor retry up to three attempts, reset only the current worker claim, and re-check deletion, approval version, job status, and content fingerprints immediately before writing derived vectors.
...
- Live hosted worker/provider verification remains an external deployment task. Locally, the Docker-free runner is configured, a real resume scan and approval completed, and the matching path now surfaces terminal embedding-provider failures. On 2026-09-16, a one-token non-sensitive `text-embedding-3-small` probe succeeded after provider credits were added; the approved resume embedding and five job embeddings completed through the local runner, and the authenticated match feed returned four ranked opportunities with no browser console errors. Remaining job embeddings can be processed incrementally to conserve credits. Before enabling paid hosted processing, deploy `process-embeddings`, configure `MATCHING_WORKER_SECRET` and a reachable `RESULENS_APP_URL`, schedule the function, run a synthetic approved-resume/job smoke test, and benchmark the HNSW index with representative data.
- The authenticated browser walkthrough verified match refresh, the four-result ranked feed, job-detail navigation, save/dismiss/applied state persistence (restored to saved after testing), relevance feedback replacement, cached top-ten explanation completion, and zero authenticated-app console errors. Explanation calls were exercised only against existing cached results to conserve credits.
...
- Account deletion is coordinated through a durable service-role-only ledger. The application marks the account first so database triggers reject new applicant work, revokes Clerk sessions, removes Storage objects through the Storage API, deletes the profile cascade and derived rows, deletes the Clerk user, and retains enough cleanup state to retry a partial failure. A fresh cleanup claim is never reset by a duplicate Clerk deletion signal.
