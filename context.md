...
- Live hosted worker/provider verification remains an external deployment task. Locally, the Docker-free runner is configured, a real resume scan and approval completed, and the matching path now surfaces terminal embedding-provider failures. A direct non-sensitive embedding check reached OpenAI and returned `credit_balance_exhausted`/`insufficient_quota`; after credits are available, click Retry Matches and run the local worker again. Before enabling paid hosted processing, deploy `process-embeddings`, configure `MATCHING_WORKER_SECRET` and a reachable `RESULENS_APP_URL`, schedule the function, run a synthetic approved-resume/job smoke test, and benchmark the HNSW index with representative data.
